...
{
    public class ChartBuilderOriginal
    {
        public ChartData Build(List<ResponseRecord> records, MessageType type)
        {
            ...
            var bucketSize = GetAdaptiveBucketSize(items);
            ...
            int p95 = GetPercentile(items, 0.95);
        ...
        private static int GetAdaptiveBucketSize(List<ResponseRecord> items)
        {
            var sorted = new List<int>();
            foreach (var r in items)
                sorted.Add(r.ResponseTime);
        ...
        private static int GetPercentile(List<ResponseRecord> items, double percentile)
        {
            var sorted = new List<int>();
            foreach (var r in items)
                sorted.Add(r.ResponseTime);
        ...
        private static void AddVerticalMarkerLine(
            double value, List<ChartBucket> buckets, SeriesCollection series,
            ...
            List<ResponseRecord> items, SeriesCollection series)
        {
            ...
            int p95 = GetPercentile(items, 0.95);
            int maxCount = GetMaxCount(buckets);
        ...
        private static int GetMaxCount(List<ChartBucket> buckets)
        {
            int maxCount = 0;
            foreach (var b in buckets)
                if ((int)b.Count > maxCount)
        ...
        private static double CalculateSpikePosition(int bucketCount, int maxCount)
        {
            // bool hasCountLabel = bucketCount >= 3;
            //
            // double offset = hasCountLabel
            //     ? Math.Max(maxCount * 0.20, 15)
            //     : Math.Max(maxCount * 0.08, 5);
            //
            // return bucketCount + offset;
            //TODO: Position of spike should be placed adaptatively - right on the top of the count number. Currently static.
